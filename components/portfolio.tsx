  ...
  return (
    ...
    >
      ...
      <div className="flex justify-center space-x-4">
        ...
        <a href="https://linkedin.com/in/danengine" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn Profile">
...
function AboutMe() {
  return (
    ...
    >
      ...
      I’m passionate about back-end development, where I focus on building robust, secure, and high-performance systems. I enjoy working with databases and APIs to ensure seamless integration, reliability, and scalability. Although back-end development is my primary focus, I also work on the front end, particularly with Next.js and shadcn/UI, to contribute to seamless user experiences. My goal is to create efficient, scalable solutions that deliver exceptional results.
      </p>
    </motion.div>
  )
}
    ...
    >
      ...
      <div className="flex flex-wrap gap-2">
        {technologies.map((tech, index) => (
          ...
          >
            <Badge variant="secondary" className="text-xs py-1 px-2 bg-gray-100 text-gray-800 hover:bg-gray-200 transition-colors flex items-center space-x-1">
              <Image src={tech.logo} alt={tech.name} width={16} height={20} />
              <span>{tech.name}</span>
            </Badge>
          </motion.div>
        ))}
...
interface Experience {
  ...
  return (
    <div className="bg-white">
      ...
      >
        <Image src={experience.logo} alt={experience.company} width={40} height={40} className="rounded-full" />
        <div className="flex-grow">
          <h3 className="font-semibold text-gray-800">{experience.company}</h3>
          <p className="text-sm text-gray-500">{experience.title} <span className="ml-2">{experience.period}</span></p>
        </div>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <p className="text-sm text-gray-700 mt-2 ml-14">
      ...
      <div className="space-y-3 mb-32">
        {education.map((edu, index) => (
          <div key={index} className="flex items-center space-x-3">
            <Image src={edu.logo} alt={edu.school} width={40} height={40} className="rounded-full" />
            <div className="flex-grow">
